...
public class FileStoreService implements StoreService {
    ...
    public FileStoreService(String pathName, ReadWriteLock lock) {
    ...
    public Account get(String id) {
        lock.readLock().lock();
        try (InnerFileStoreService innerFileStoreService = new InnerFileStoreService(pathName)) {
            return innerFileStoreService.get(id);
        } catch (IOException e) {
            e.printStackTrace();
        } finally {
    ...
    public Collection<Account> get() {
        lock.readLock().lock();
        try (InnerFileStoreService innerFileStoreService = new InnerFileStoreService(pathName)) {
            return innerFileStoreService.get();
        } catch (IOException e) {
            e.printStackTrace();
        } finally {
            lock.readLock().unlock();
    ...
    public void delete(String id) {
        lock.writeLock().lock();
        try (InnerFileStoreService innerFileStoreService = new InnerFileStoreService(pathName)) {
            innerFileStoreService.delete(id);
        } catch (IOException e) {
            e.printStackTrace();
        } finally {
    ...
    public void insert(Account account) {
        lock.writeLock().lock();
        try (InnerFileStoreService innerFileStoreService = new InnerFileStoreService(pathName)) {
            innerFileStoreService.insert(account);
        } catch (IOException e) {
            e.printStackTrace();
        } finally {
            lock.writeLock().unlock();
    ...
    public void update(Account account) {
        lock.writeLock().lock();
        try (InnerFileStoreService innerFileStoreService = new InnerFileStoreService(pathName)) {
            innerFileStoreService.update(account);
        } catch (IOException e) {
            e.printStackTrace();
        } finally {
            lock.writeLock().unlock();
    ...
    private class InnerFileStoreService extends RandomAccessFile implements StoreService {
        private final int STR_MIN_SIZE = 150;

        InnerFileStoreService(String pathName)
    ...
    public static void main(String[] args) throws IOException {
        ...
        FileStoreService service = new FileStoreService("StoreFile.csv", lock);
        int i = 1000;
        ...
        Thread t1 = new Thread(new GetAndUpdateThread("StoreFile.csv", lock));
        t1.start();
        ...
        account20.setHolder("changed in main thread");
        service.update(account20);
        service.get().forEach(System.out::println);
    }

    static class GetAndUpdateThread implements Runnable{
        ReadWriteLock lock;
        String pathName;
        GetAndUpdateThread(String pathName, ReadWriteLock lock){
            this.pathName = pathName;
            this.lock = lock;
        }
        @Override
        public void run(){
            FileStoreService service = new FileStoreService(pathName, lock);
            Account account20 = service.get("20");
            account20.setHolder("changed in GetAndUpdateThread");
            service.update(account20);
            service.get().forEach(x -> System.out.println("GetAndUpdateThread: " + x));
        }
    }
}
